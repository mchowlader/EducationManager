...
public class RemoveUserRoleCommandHandler(
    ...
    : IRequestHandler<RemoveUserRoleCommand, Result<object>>
{
    private readonly IUserRoleRepository _userRoleRepository = userRoleRepository;
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    public async Task<Result<object>> Handle(RemoveUserRoleCommand request, CancellationToken cancellationToken)
    {
        var userRole = await _userRoleRepository.GetByUserIdAndRoleIdAsync(request.UserId, request.RoleId, cancellationToken);

        if (userRole is null)
            return Result<object>.Failure("User role not found.");

        userRole.IsDelete = true;
        _userRoleRepository.Update(userRole);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        return Result<object>.Success(true, "Role removed successfully.");
    }
}
